...
export default class CanvasSelect extends EventBus {
  ...
  /** 是否启用左键拖动画布 */
  enableLeftDragCanvas: boolean = true;
  /** 最小矩形宽度 */
  MIN_WIDTH = 10;
  /** 最小矩形高度 */
  MIN_HEIGHT = 10;
  ...
  constructor(
    ...
    options: { enableLeftDragCanvas?: boolean } = {}
  ) {
    super();
    ...
    if (container instanceof HTMLCanvasElement) {
      ...
      this.enableLeftDragCanvas = options.enableLeftDragCanvas ?? false;
      this.initSetting();
      this.initEvents();
    ...
    this.timer = window.requestAnimationFrame(() => {
      ...
      if (this.IMAGE_WIDTH && this.IMAGE_HEIGHT) {
        ...
        if (this.image.width > 1) {
          ...
          this.ctx.fillRect(0, 0, this.IMAGE_WIDTH, this.IMAGE_HEIGHT);

          // 绘制边框
          this.ctx.strokeStyle = "#cccccc";
          this.ctx.lineWidth = 1;
          this.ctx.strokeRect(0, 0, this.IMAGE_WIDTH, this.IMAGE_HEIGHT);
        }
      }
  ...
  initEmptyCanvas(
    ...
    this.IMAGE_ORIGIN_WIDTH = this.IMAGE_WIDTH = width;
    this.IMAGE_ORIGIN_HEIGHT = this.IMAGE_HEIGHT = height;

    // 创建一个1x1像素的透明背景图片作为占位符

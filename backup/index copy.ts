...
export default class CanvasSelect extends EventBus {
  ...
  offScreen: any = undefined;

  offScreenCtx: any = undefined;

  // 放大镜相关配置 Start
  magnifierCanvas: any = undefined;
  magnifierCtx: any = undefined;
  // 默认展示放大镜
   ...
   * @param src image src
   */
  // el: HTMLCanvasElement | string, src?:  string | HTMLImageElement
  constructor(el: any, src: any) {
    super();
  ...
  /** 当前当前选中的标注 */
  get activeShape() {
    return this.dataset.find((x: any) => x.active) || {};
  }
  ...
  /* 提取像素信息 */
  getImageDataFromCanvas(canvas: any, [x, y, width, height]: any) {
    const context = canvas.getContext("2d", { willReadFrequently: true });
    return context?.getImageData(x, y, width, height);
  }

  /** 合成事件 e: TouchEvent | MouseEvent*/
  mergeEvent(e: any) {
    let mouseX = 0;
    let mouseY = 0;
    let mouseCX = 0;
    let mouseCY = 0;
    if (this.isMobile && e.touches) {
  ...
  handleMouseDown(e: any) {
    ...
    this.mouse =
      this.isMobile && e.touches?.length === 2
        ? [mouseCX, mouseCY]
        : [mouseX, mouseY];
    this.remmberOrigin = [mouseX - this.originX, mouseY - this.originY];
    if (
      (!this.isMobile && e.buttons === 1) ||
      (this.isMobile && e.touches?.length === 1)
    ) {
      // 鼠标左键
      const ctrls = this.activeShape.ctrlsData || [];
      this.ctrlIndex = ctrls.findIndex((coor: any) =>
      ...
      if (this.ctrlIndex > -1 && !this.readonly) {
        ...
        this.remmber = [[offsetX - x0, offsetY - y0]] as any;
      } else if (this.isInBackground(e)) {
        if (this.activeShape.creating && !this.readonly) {
          // 创建中
          if ([2, 5].includes(this.activeShape.type)) {
            const [x, y] =
          ...
          if (hitShapeIndex > -1 && hitShape) {
            ...
            this.dataset.forEach(
              (item: any, i: number) => (item.active = i === hitShapeIndex)
            );
            ...
            if (!this.readonly) {
              ...
              if ([Shape.Dot, Shape.Circle].includes(hitShape.type)) {
                ...
                this.remmber = [[offsetX - x, offsetY - y]] as any;
              } else {
                hitShape.coor.forEach((pt: any) => {
                  this.remmber.push([offsetX - pt[0], offsetY - pt[1]]) as any;
                });
              }
            }
            ...
            this.dataset.sort((a: any, b: any) => a.index - b.index);
      ...
      (!this.isMobile && e.buttons === 2) ||
      (this.isMobile && e.touches?.length === 3 && !this.readonly)
    ) {
      // 鼠标右键
  ...
  handleMouseMove(e: any) {
    ...
    if (
      ...
      // e.buttons === 2 && e.which === 3 鼠标右键
      (!this.isMobile && e.buttons === 2 && e.which === 3) ||
      (e.buttons === 1 && this.readonly) ||
      (this.isMobile && e.touches?.length === 1 && !this.isTouch2)
    ) {
      console.log("拖动背景");
      // 拖动背景
  ...
  handleDblclick(e: any) {
    ...
    if ([Shape.Polygon, Shape.Line].includes(this.activeShape.type)) {
      ...
      if (this.activeShape.active) {
        this.activeShape.gridRects.forEach((rect: any) => {
   ...
   */
  setImage(source: any) {
    console.log("setImage~~~~图像路径", source);
    ...
    for (let i = this.dataset.length - 1; i > -1; i--) {
      ...
      if (
        (shape.type === Shape.Dot &&
          this.isPointInCircle(mousePoint, shape.coor, this.ctrlRadius)) ||
        ...
        (shape.type === Shape.Rect &&
          this.isPointInRect(mousePoint, shape.coor)) ||
        (shape.type === Shape.Polygon &&
          this.isPointInPolygon(mousePoint, shape.coor)) ||
        (shape.type === Shape.Line &&
          this.isPointInLine(mousePoint, shape.coor)) ||
   ...
   * @returns 布尔值
   */
  isPointInRect(point: any, coor: any) {
    const [x, y] = point;
    const [[x0, y0], [x1, y1]] = coor.map((a: any) =>
   ...
   * @returns 布尔值
   */
  isPointInPolygon(point: any, coor: any) {
    if (!this.offScreenCtx) return false;
    this.offScreenCtx.save();
    this.offScreenCtx.clearRect(0, 0, this.WIDTH, this.HEIGHT);
    this.offScreenCtx.translate(this.originX, this.originY);
    this.offScreenCtx.beginPath();
    coor.forEach((pt: any, i: number) => {
      const [x, y] = pt.map((a: any) => Math.round(a * this.scale));
      if (i === 0) {
   ...
   * @returns 布尔值
   */
  isPointInCircle(point: any, center: any, r: number) {
    const [x, y] = point;
    const [x0, y0] = center.map((a: any) => a * this.scale);
   ...
   * @returns 布尔值
   */
  isPointInLine(point: any, coor: any) {
    if (!this.offScreenCtx) return false;
    this.offScreenCtx.save();
    this.offScreenCtx.clearRect(0, 0, this.WIDTH, this.HEIGHT);
    this.offScreenCtx.translate(this.originX, this.originY);
  ...
  drawPolygon(shape: any) {
    ...
    this.ctx.strokeStyle =
      active || creating
        ...
        : strokeStyle || this.strokeStyle;
    this.ctx.beginPath();
    coor.forEach((el: any, i: number) => {
      const [x, y] = el.map((a: any) => Math.round(a * this.scale));
      if (i === 0) {
        this.ctx?.moveTo(x, y);
      } else {
        this.ctx?.lineTo(x, y);
  ...
  drawDot(shape: any) {
    ...
    const [x, y] = coor.map((a: any) => a * this.scale);
    this.ctx.save();
    this.ctx.lineWidth = lineWidth || this.lineWidth;
    this.ctx.fillStyle =
      active || creating
  ...
  drawGrid(shape: any) {
    ...
    this.ctx.strokeStyle =
      active || creating
        ? this.activeStrokeStyle
        : strokeStyle || this.strokeStyle;
    shape.gridRects.forEach((rect: any, m: number) => {
      this.drawRect(rect, {
        selectedFillStyle:
  ...
  drawCtrl(point: any) {
    if (!this.ctx) return;
    const [x, y] = point.map((a: any) => a * this.scale);
    this.ctx.save();
    this.ctx.beginPath();
   ...
   * @param shape 标注实例
   */
  drawCtrlList(shape: any) {
    shape.ctrlsData.forEach((point: any, i: number) => {
  ...
  deleteByIndex(index: number) {
    const num = this.dataset.findIndex((x: any) => x.index === index);
    if (num > -1) {
  ...
  deleteByUuid(uuid: string) {
    const target = this.dataset.find((x: any) => x.uuid === uuid);
    if (target) {
      this.emit("delete", target);
      this.dataset = this.dataset.filter((x: any) => x.uuid !== uuid);
      this.update();
    }
  }
